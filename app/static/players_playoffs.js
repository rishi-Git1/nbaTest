...
async function loadData() {
  ...
  try {
    const res = await fetch(`/api/players-playoffs?${params.toString()}`);
    if (!res.ok) {
    ...
    statusEl.textContent = `Loaded ${payload.data.length} playoff players (total ${payload.meta.total}) for ${payload.meta.season}`;
  } catch (error) {
